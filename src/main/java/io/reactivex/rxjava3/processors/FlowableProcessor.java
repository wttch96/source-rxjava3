...
/**
 * Represents a Subscriber and a Flowable (Publisher) at the same time, allowing multicasting events
 * from a single source to multiple child Subscribers.
 *
 * <p>All methods except the onSubscribe, onNext, onError and onComplete are thread-safe. Use {@link
 * #toSerialized()} to make these methods thread-safe as well.
 *
 * @param <T> the item value type
 */
public abstract class FlowableProcessor<@NonNull T> extends Flowable<T>
  ...
  /**
   * Returns the error that caused the FlowableProcessor to terminate or null if the
   * FlowableProcessor hasn't terminated yet.
   *
   * <p>The method is thread-safe.
   *
   * @return the error that caused the FlowableProcessor to terminate or null if the
   *     FlowableProcessor hasn't terminated yet
   */
  @Nullable
  @CheckReturnValue
