 ...
 *
 * <p>You can override the {@link #onSubscribe(Subscription)}, {@link #onNext(Object)}, {@link
 * #onError(Throwable)} and {@link #onComplete()} methods but not the others (this is by design).
 *
 * <p>When calling the default request method, you are requesting on behalf of the wrapped actual
 * {@link Subscriber} if any.
 *
...
public class TestSubscriber<T> extends BaseTestConsumer<T, TestSubscriber<T>>
  ...
  /**
   * Constructs a non-forwarding {@code TestSubscriber} with an initial request value of {@link
   * Long#MAX_VALUE}.
   */
  ...
  /**
   * Constructs a forwarding {@code TestSubscriber} but leaves the requesting to the wrapped {@link
   * Subscriber}.
   *
  ...
  /**
   * Constructs a forwarding {@code TestSubscriber} with the specified initial request amount and an
   * actual {@link Subscriber} to forward events to.
   *
   * <p>The {@code TestSubscriber} doesn't validate the initialRequest value so one can test sources
   * with invalid values as well.
   *
  ...
  /**
   * Returns true if this {@code TestSubscriber} received a {@link Subscription} via {@link
   * #onSubscribe(Subscription)}.
   *
   * @return true if this {@code TestSubscriber} received a {@link Subscription} via {@link
   *     #onSubscribe(Subscription)}
   */
