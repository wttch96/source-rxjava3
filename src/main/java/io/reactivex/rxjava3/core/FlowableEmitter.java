...
/**
 * Abstraction over a Reactive Streams {@link org.reactivestreams.Subscriber} that allows
 * associating a resource with it and exposes the current number of downstream requested amount.
 *
 * <p>The {@link #onNext(Object)}, {@link #onError(Throwable)}, {@link #tryOnError(Throwable)} and
 * {@link #onComplete()} methods should be called in a sequential manner, just like the {@link
 * org.reactivestreams.Subscriber Subscriber}'s methods. Use the {@code FlowableEmitter} the {@link
 * #serialize()} method returns instead of the original {@code FlowableEmitter} instance provided by
 * the generator routine if you want to ensure this. The other methods are thread-safe.
 *
...
public interface FlowableEmitter<@NonNull T> extends Emitter<T> {
  ...
  /**
   * Sets a {@link Cancellable} on this emitter; any previous {@link Disposable} or {@code
   * Cancellable} will be disposed/cancelled.
   *
   * <p>This method is thread-safe.
   *
  ...
  /**
   * Returns true if the downstream cancelled the sequence or the emitter was terminated via {@link
  ...
  /**
   * Ensures that calls to {@code onNext}, {@code onError} and {@code onComplete} are properly
   * serialized.
   *
