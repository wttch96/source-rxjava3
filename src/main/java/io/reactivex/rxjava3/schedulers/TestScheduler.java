...
/**
 * A special, non thread-safe scheduler for testing operators that require a scheduler without
 * introducing real concurrency and allows manually advancing a virtual time.
 */
public final class TestScheduler extends Scheduler {
  ...
  /**
   * Triggers any actions that have not yet been triggered and that are scheduled to be triggered at
   * or before this Scheduler's present time.
   */
  ...
  final class TestWorker extends Worker {
    ...
    public Disposable schedule(@NonNull Runnable run, long delayTime, @NonNull TimeUnit unit) {
      ...
      final TimedRunnable timedAction =
          new TimedRunnable(this, time + unit.toNanos(delayTime), run, counter++);
