...
public final class ObservableFlatMapStream<T, R> extends Observable<R> {
  ...
  public ObservableFlatMapStream(
      Observable<T> source, Function<? super T, ? extends Stream<? extends R>> mapper) {
    this.source = source;
    this.mapper = mapper;
  }
          ...
          while (it.hasNext()) {
            ...
            R value =
                Objects.requireNonNull(
                    it.next(), "The Stream's Iterator.next returned a null value");
            if (disposed) {
              done = true;
              break;
            }
