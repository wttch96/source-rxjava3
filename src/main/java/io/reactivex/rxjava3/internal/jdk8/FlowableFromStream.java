...
public final class FlowableFromStream<T> extends Flowable<T> {
  ...
  /**
   * Subscribes to the Stream by picking the normal or conditional stream Subscription
   * implementation.
   *
  ...
  public static <T> void subscribeStream(Subscriber<? super T> s, Stream<T> stream) {
    ...
    if (s instanceof ConditionalSubscriber) {
      s.onSubscribe(
          new StreamConditionalSubscription<>(
              (ConditionalSubscriber<? super T>) s, iterator, stream));
    } else {
  ...
  abstract static class AbstractStreamSubscription<T> extends AtomicLong
    ...
    public T poll() {
      ...
      return Objects.requireNonNull(
          iterator.next(), "The Stream's Iterator.next() returned a null value");
    }
  ...
  static final class StreamSubscription<T> extends AbstractStreamSubscription<T> {
    ...
    StreamSubscription(
        Subscriber<? super T> downstream, Iterator<T> iterator, AutoCloseable closeable) {
          ...
          try {
            next =
                Objects.requireNonNull(
                    iterator.next(), "The Stream's Iterator returned a null value");
          } catch (Throwable ex) {
            Exceptions.throwIfFatal(ex);
  ...
  static final class StreamConditionalSubscription<T> extends AbstractStreamSubscription<T> {
    ...
    StreamConditionalSubscription(
        ConditionalSubscriber<? super T> downstream,
        Iterator<T> iterator,
        AutoCloseable closeable) {
      super(iterator, closeable);
      this.downstream = downstream;
