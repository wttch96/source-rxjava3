...
public final class FlowableMapOptional<T, R> extends Flowable<R> {
  ...
  public FlowableMapOptional(
      Flowable<T> source, Function<? super T, Optional<? extends R>> mapper) {
    this.source = source;
    this.mapper = mapper;
  }
  ...
  protected void subscribeActual(Subscriber<? super R> s) {
    if (s instanceof ConditionalSubscriber) {
      source.subscribe(
          new MapOptionalConditionalSubscriber<>((ConditionalSubscriber<? super R>) s, mapper));
    } else {
  ...
  static final class MapOptionalSubscriber<T, R> extends BasicFuseableSubscriber<T, R>
    ...
    MapOptionalSubscriber(
        Subscriber<? super R> downstream, Function<? super T, Optional<? extends R>> mapper) {
      super(downstream);
      this.mapper = mapper;
    }
    ...
    public R poll() throws Throwable {
      for (; ; ) {
        ...
        Optional<? extends R> result =
            Objects.requireNonNull(mapper.apply(item), "The mapper returned a null Optional");
        if (result.isPresent()) {
          return result.get();
        }
        if (sourceMode == ASYNC) {
  ...
  static final class MapOptionalConditionalSubscriber<T, R>
    ...
    MapOptionalConditionalSubscriber(
        ConditionalSubscriber<? super R> downstream,
