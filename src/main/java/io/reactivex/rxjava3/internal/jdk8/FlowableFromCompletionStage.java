...
public final class FlowableFromCompletionStage<T> extends Flowable<T> {
  ...
  static final class CompletionStageHandler<T> extends DeferredScalarSubscription<T>
    ...
    CompletionStageHandler(
        Subscriber<? super T> downstream, BiConsumerAtomicReference<T> whenReference) {
      super(downstream);
