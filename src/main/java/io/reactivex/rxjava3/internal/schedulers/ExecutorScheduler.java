  ...
  public static final class ExecutorWorker extends Scheduler.Worker implements Runnable {
    ...
    public Disposable schedule(@NonNull Runnable run, long delay, @NonNull TimeUnit unit) {
      ...
      if (executor instanceof ScheduledExecutorService) {
        try {
          Future<?> f =
              ((ScheduledExecutorService) executor).schedule((Callable<Object>) sr, delay, unit);
          sr.setFuture(f);
        } catch (RejectedExecutionException ex) {
          disposed = true;
          RxJavaPlugins.onError(ex);
    ...
    /**
     * Wrapper for a {@link Runnable} with additional logic for handling interruption on a shared
     * thread, similar to how Java Executors do it.
     */
