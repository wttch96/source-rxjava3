 ...
 * are actually done. This makes it possible to layer additional behavior on this {@link Scheduler}.
 * The only parameter is a function that flattens an {@link Observable} of {@link Observable} of
 * {@link Completable}s into just one {@link Completable}. There must be a chain of operators
 * connecting the returned value to the source {@link Observable} otherwise any work scheduled on
 * the returned {@link Scheduler} will not be executed.
 *
 * <p>When {@link Scheduler#createWorker()} is invoked a {@link Observable} of {@link Completable}s
 * is onNext'd to the combinator to be flattened. If the inner {@link Observable} is not immediately
 * subscribed to an calls to {@link Worker#schedule} are buffered. Once the {@link Observable} is
 ...
 *
 * <p>When the {@link io.reactivex.rxjava3.core.Scheduler.Worker Worker} is unsubscribed the {@link
 * Completable} emits an onComplete and triggers any behavior in the flattening operator. The {@link
 * Observable} and all {@link Completable}s give to the flattening function never onError.
 *
 ...
 * }</pre>
 *
 * <p>This is a slightly different way to limit the concurrency but it has some interesting benefits
 * and drawbacks to the method above. It works by limited the number of concurrent {@link
 * io.reactivex.rxjava3.core.Scheduler.Worker Worker}s rather than individual actions. Generally
 * each {@link Observable} uses its own {@link io.reactivex.rxjava3.core.Scheduler.Worker Worker}.
 * This means that this will essentially limit the number of concurrent subscribes. The danger comes
 * from using operators like {@link Flowable#zip(org.reactivestreams.Publisher,
 * org.reactivestreams.Publisher, io.reactivex.rxjava3.functions.BiFunction)} where subscribing to
 * the first {@link Observable} could deadlock the subscription to the second.
 *
 * <pre>{@code
 * Scheduler limitScheduler = Schedulers.computation().when(workers -> {
 *  // use merge max concurrent to limit the number of concurrent
 ...
 * }</pre>
 *
 * Slowing down the rate to no more than than 1 a second. This suffers from the same problem as the
 * one above I could find an {@link Observable} operator that limits the rate without dropping the
 * values (aka leaky bucket algorithm).
 *
 * <pre>{@code
...
public class SchedulerWhen extends Scheduler implements Disposable {
  ...
  public Worker createWorker() {
    ...
    final FlowableProcessor<ScheduledAction> actionProcessor =
        UnicastProcessor.<ScheduledAction>create().toSerialized();
  ...
  abstract static class ScheduledAction extends AtomicReference<Disposable> implements Disposable {
    ...
    protected abstract Disposable callActual(
        Worker actualWorker, CompletableObserver actionCompletable);
  ...
  static class DelayedAction extends ScheduledAction {
    ...
    @Override
    protected Disposable callActual(Worker actualWorker, CompletableObserver actionCompletable) {
      return actualWorker.schedule(
          new OnCompletedAction(action, actionCompletable), delayTime, unit);
    }
  }
  ...
  static final class QueueWorker extends Worker {
    ...
    public Disposable schedule(
        @NonNull final Runnable action, final long delayTime, @NonNull final TimeUnit unit) {
