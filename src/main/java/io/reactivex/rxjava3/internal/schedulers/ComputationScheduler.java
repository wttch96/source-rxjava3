...
/**
 * Holds a fixed pool of worker threads and assigns them to requested Scheduler.Workers in a
 * round-robin fashion.
 */
public final class ComputationScheduler extends Scheduler implements SchedulerMultiWorkerSupport {
  ...
  /**
   * Key to setting the maximum number of computation scheduler threads. Zero or less is interpreted
   * as use available. Capped by available.
   */
  ...
  static {
    MAX_THREADS =
        cap(Runtime.getRuntime().availableProcessors(), Integer.getInteger(KEY_MAX_THREADS, 0));
  ...
  /**
   * Create a scheduler with pool size equal to the available processor count and using least-recent
  ...
  public Disposable schedulePeriodicallyDirect(
      @NonNull Runnable run, long initialDelay, long period, TimeUnit unit) {
    PoolWorker w = pool.get().getEventLoop();
