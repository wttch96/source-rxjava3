...
public final class ObservableTimeoutTimed<T> extends AbstractObservableWithUpstream<T, T> {
  ...
  protected void subscribeActual(Observer<? super T> observer) {
    if (other == null) {
      TimeoutObserver<T> parent =
          new TimeoutObserver<>(observer, timeout, unit, scheduler.createWorker());
      observer.onSubscribe(parent);
      parent.startTimeout(0L);
      source.subscribe(parent);
    } else {
      TimeoutFallbackObserver<T> parent =
          new TimeoutFallbackObserver<>(observer, timeout, unit, scheduler.createWorker(), other);
      observer.onSubscribe(parent);
      parent.startTimeout(0L);
      source.subscribe(parent);
    }
  ...
  static final class TimeoutObserver<T> extends AtomicLong
    ...
    TimeoutObserver(
        Observer<? super T> actual, long timeout, TimeUnit unit, Scheduler.Worker worker) {
