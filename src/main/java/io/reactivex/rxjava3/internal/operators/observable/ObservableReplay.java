...
public final class ObservableReplay<T> extends ConnectableObservable<T>
   ...
   * @param connectableFactory the factory that returns a ConnectableObservable for each individual
   *     subscriber
   * @param selector the function that receives an Observable and should return another Observable
  ...
  public static <T> ConnectableObservable<T> create(
      ObservableSource<T> source, final int bufferSize, boolean eagerTruncate) {
  ...
  public static <T> ConnectableObservable<T> create(
      ...
      boolean eagerTruncate) {
    return create(
        source, new ScheduledReplaySupplier<>(bufferSize, maxAge, unit, scheduler, eagerTruncate));
  }
   ...
   */
  static <T> ConnectableObservable<T> create(
      ObservableSource<T> source, final BufferSupplier<T> bufferFactory) {
    ...
    return RxJavaPlugins.onAssembly(
        new ObservableReplay<>(onSubscribe, source, curr, bufferFactory));
  }

  private ObservableReplay(
      ObservableSource<T> onSubscribe,
      ObservableSource<T> source,
      final AtomicReference<ReplayObserver<T>> current,
      final BufferSupplier<T> bufferFactory) {
  ...
  @SuppressWarnings("rawtypes")
  static final class ReplayObserver<T> extends AtomicReference<Disposable>
    ...
    /**
     * Atomically try adding a new InnerDisposable to this Observer or return false if this Observer
     * was terminated.
     *
  ...
  static final class InnerDisposable<T> extends AtomicInteger implements Disposable {
    ...
    /**
     * The parent subscriber-to-source used to allow removing the child in case of child dispose()
     * call.
     */
      ...
      try {
        co =
            Objects.requireNonNull(
                ...
                "The connectableFactory returned a null ConnectableObservable");
        observable =
            Objects.requireNonNull(
                selector.apply(co), "The selector returned a null ObservableSource");
      } catch (Throwable e) {
