...
import io.reactivex.rxjava3.internal.operators.flowable.FlowableConcatMap.ConcatMapInner;
import io.reactivex.rxjava3.internal.operators.flowable.FlowableConcatMap.ConcatMapSupport;
...
public final class FlowableConcatMapScheduler<T, R> extends AbstractFlowableWithUpstream<T, R> {
  ...
  public FlowableConcatMapScheduler(
      Flowable<T> source,
  ...
  protected void subscribeActual(Subscriber<? super R> s) {
    switch (errorMode) {
      case BOUNDARY:
        source.subscribe(
            new ConcatMapDelayed<>(s, mapper, prefetch, false, scheduler.createWorker()));
        break;
      case END:
        source.subscribe(
            new ConcatMapDelayed<>(s, mapper, prefetch, true, scheduler.createWorker()));
        break;
      default:
  ...
  abstract static class BaseConcatMapSubscriber<T, R> extends AtomicInteger
