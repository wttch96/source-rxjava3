...
public final class FlowableWindowTimed<T> extends AbstractFlowableWithUpstream<T, Flowable<T>> {
  ...
  protected void subscribeActual(Subscriber<? super Flowable<T>> downstream) {
    if (timespan == timeskip) {
      if (maxSize == Long.MAX_VALUE) {
        source.subscribe(
            new WindowExactUnboundedSubscriber<>(
                downstream, timespan, unit, scheduler, bufferSize));
        return;
      }
      source.subscribe(
          new WindowExactBoundedSubscriber<>(
              downstream, timespan, unit, scheduler, bufferSize, maxSize, restartTimerOnMaxSize));
      return;
    }
    source.subscribe(
        new WindowSkipSubscriber<>(
            downstream, timespan, timeskip, unit, scheduler.createWorker(), bufferSize));
  }

  abstract static class AbstractWindowSubscriber<T> extends AtomicInteger
    ...
    AbstractWindowSubscriber(
        Subscriber<? super Flowable<T>> downstream, long timespan, TimeUnit unit, int bufferSize) {
  ...
  static final class WindowExactUnboundedSubscriber<T> extends AbstractWindowSubscriber<T>
      implements Runnable {
    ...
    void createFirstWindow() {
      if (!downstreamCancelled.get()) {
        if (requested.get() != 0L) {
          ...
          FlowableWindowSubscribeIntercept<T> intercept =
              new FlowableWindowSubscribeIntercept<>(window);
            ...
            if (o == NEXT_WINDOW) {
              ...
              if (downstreamCancelled.get()) {
                ...
                if (requested.get() == emitted) {
                  ...
                  downstream.onError(
                      new MissingBackpressureException(missingBackpressureMessage(emitted)));
                } else {
                  emitted++;
  ...
  static final class WindowExactBoundedSubscriber<T> extends AbstractWindowSubscriber<T>
      implements Runnable {
      ...
      if (!downstreamCancelled.get()) {
        if (requested.get() != 0L) {
          ...
          if (restartTimerOnMaxSize) {
            ...
            timer.replace(
                scheduler.schedulePeriodicallyDirect(boundaryTask, timespan, timespan, unit));
          }
          ...
          if (restartTimerOnMaxSize) {
            timer.update(
                worker.schedulePeriodically(
                    new WindowBoundaryRunnable(this, emitted), timespan, timespan, unit));
          }
  ...
  static final class WindowSkipSubscriber<T> extends AbstractWindowSubscriber<T>
      implements Runnable {
    ...
    void createFirstWindow() {
      if (!downstreamCancelled.get()) {
        if (requested.get() != 0L) {
          ...
          worker.schedulePeriodically(
              new WindowBoundaryRunnable(this, true), timeskip, timeskip, unit);
  ...
  static String missingBackpressureMessage(long index) {
    return "Unable to emit the next window (#"
        + index
