...
/**
 * Maps a source item to another SingleSource then calls a BiFunction with the original item and the
 * secondary item to generate the final result.
 *
...
public final class SingleFlatMapBiSelector<T, U, R> extends Single<R> {
  ...
  public SingleFlatMapBiSelector(
  ...
  static final class FlatMapBiMainObserver<T, U, R> implements SingleObserver<T>, Disposable {
    ...
    public void onSuccess(T value) {
      ...
      try {
        next =
            Objects.requireNonNull(mapper.apply(value), "The mapper returned a null MaybeSource");
      } catch (Throwable ex) {
    ...
    static final class InnerObserver<T, U, R> extends AtomicReference<Disposable>
        ...
        try {
          r =
              Objects.requireNonNull(
                  resultSelector.apply(t, value), "The resultSelector returned a null value");
        } catch (Throwable ex) {
          Exceptions.throwIfFatal(ex);
