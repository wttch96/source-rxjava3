...
/**
 * Shares a single underlying connection to the upstream ObservableSource and multicasts events to
 * all subscribed observers until the upstream completes or the connection is disposed.
 *
 * <p>The difference to ObservablePublish is that when the upstream terminates, late observers will
 * receive that terminal event until the connection is disposed and the ConnectableObservable is
 * reset to its fresh state.
 *
...
public final class ObservablePublish<T> extends ConnectableObservable<T>
  ...
  static final class PublishConnection<T> extends AtomicReference<InnerDisposable<T>[]>
  ...
  /**
   * Intercepts the dispose signal from the downstream and removes itself from the connection's
   * observers array at most once.
   *
   * @param <T> the element type
   */
  static final class InnerDisposable<T> extends AtomicReference<PublishConnection<T>>
