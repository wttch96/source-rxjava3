...
/**
 * Measures the time between subscription and the success item emission from the upstream and emits
 * this as a {@link Timed} success value.
 *
...
public final class SingleTimeInterval<T> extends Single<Timed<T>> {
  ...
  public SingleTimeInterval(
      SingleSource<T> source, TimeUnit unit, Scheduler scheduler, boolean start) {
    this.source = source;
    this.unit = unit;
  ...
  static final class TimeIntervalSingleObserver<T> implements SingleObserver<T>, Disposable {
    ...
    TimeIntervalSingleObserver(
        SingleObserver<? super Timed<T>> downstream,
        TimeUnit unit,
        Scheduler scheduler,
        boolean start) {
