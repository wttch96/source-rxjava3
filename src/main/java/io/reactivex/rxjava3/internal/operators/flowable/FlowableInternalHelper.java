...
public final class FlowableInternalHelper {
  ...
  public static <T, S> BiFunction<S, Emitter<T>, S> simpleBiGenerator(
  ...
  static final class ItemDelayFunction<T, U> implements Function<T, Publisher<T>> {
    ...
    public Publisher<T> apply(final T v) throws Throwable {
      Publisher<U> p =
          Objects.requireNonNull(itemDelay.apply(v), "The itemDelay returned a null Publisher");
      return new FlowableTakePublisher<>(p, 1).map(Functions.justFunction(v)).defaultIfEmpty(v);
    }
  }

  public static <T, U> Function<T, Publisher<T>> itemDelay(
      final Function<? super T, ? extends Publisher<U>> itemDelay) {
  ...
  static final class FlatMapWithCombinerOuter<T, R, U> implements Function<T, Publisher<R>> {
    ...
    public Publisher<R> apply(final T t) throws Throwable {
      ...
      Publisher<U> u =
          (Publisher<U>)
              Objects.requireNonNull(mapper.apply(t), "The mapper returned a null Publisher");
  ...
  static final class FlatMapIntoIterable<T, U> implements Function<T, Publisher<U>> {
    ...
    public Publisher<U> apply(T t) throws Throwable {
      return new FlowableFromIterable<>(
          Objects.requireNonNull(mapper.apply(t), "The mapper returned a null Iterable"));
    }
  }

  public static <T, U> Function<T, Publisher<U>> flatMapIntoIterable(
  ...
  public static <T> Supplier<ConnectableFlowable<T>> replaySupplier(
      final Flowable<T> parent, final int bufferSize, boolean eagerTruncate) {
  ...
  static final class TimedReplay<T> implements Supplier<ConnectableFlowable<T>> {
    ...
    TimedReplay(
        Flowable<T> parent, long time, TimeUnit unit, Scheduler scheduler, boolean eagerTruncate) {
      this.parent = parent;
      this.time = time;
      this.unit = unit;
      this.scheduler = scheduler;
