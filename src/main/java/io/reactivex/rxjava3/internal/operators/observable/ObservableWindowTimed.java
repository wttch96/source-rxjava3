...
public final class ObservableWindowTimed<T>
  ...
  protected void subscribeActual(Observer<? super Observable<T>> downstream) {
    if (timespan == timeskip) {
      if (maxSize == Long.MAX_VALUE) {
        source.subscribe(
            new WindowExactUnboundedObserver<>(downstream, timespan, unit, scheduler, bufferSize));
        return;
      }
      source.subscribe(
          new WindowExactBoundedObserver<>(
              downstream, timespan, unit, scheduler, bufferSize, maxSize, restartTimerOnMaxSize));
      return;
    }
    source.subscribe(
        new WindowSkipObserver<>(
            downstream, timespan, timeskip, unit, scheduler.createWorker(), bufferSize));
  }

  abstract static class AbstractWindowObserver<T> extends AtomicInteger
    ...
    AbstractWindowObserver(
        Observer<? super Observable<T>> downstream, long timespan, TimeUnit unit, int bufferSize) {
  ...
  static final class WindowExactUnboundedObserver<T> extends AbstractWindowObserver<T>
      implements Runnable {
    ...
    void createFirstWindow() {
      if (!downstreamCancelled.get()) {
        ...
        ObservableWindowSubscribeIntercept<T> intercept =
            new ObservableWindowSubscribeIntercept<>(window);
  ...
  static final class WindowExactBoundedObserver<T> extends AbstractWindowObserver<T>
      implements Runnable {
