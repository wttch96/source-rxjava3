...
public final class FlowableOnBackpressureReduce<T> extends AbstractFlowableWithUpstream<T, T> {
  ...
  public FlowableOnBackpressureReduce(
      @NonNull Flowable<T> source, @NonNull BiFunction<T, T, T> reducer) {
    super(source);
  ...
  static final class BackpressureReduceSubscriber<T>
    ...
    BackpressureReduceSubscriber(
        @NonNull Subscriber<? super T> downstream, @NonNull BiFunction<T, T, T> reducer) {
      super(downstream);
      this.reducer = reducer;
    }
    ...
    public void onNext(T t) {
      ...
      if (v == null) {
        ...
        try {
          current.lazySet(
              Objects.requireNonNull(reducer.apply(v, t), "The reducer returned a null value"));
        } catch (Throwable ex) {
