...
public final class ObservableInternalHelper {
    ...
    public ObservableSource<T> apply(final T v) throws Throwable {
      ObservableSource<U> o =
          Objects.requireNonNull(
              itemDelay.apply(v), "The itemDelay returned a null ObservableSource");
      return new ObservableTake<>(o, 1).map(Functions.justFunction(v)).defaultIfEmpty(v);
    }
  }

  public static <T, U> Function<T, ObservableSource<T>> itemDelay(
      final Function<? super T, ? extends ObservableSource<U>> itemDelay) {
  ...
  public static <T, U> Function<T, ObservableSource<U>> flatMapIntoIterable(
  ...
  public static <T> Supplier<ConnectableObservable<T>> replaySupplier(
      final Observable<T> parent, final int bufferSize, boolean eagerTruncate) {
  ...
  public static <T> Supplier<ConnectableObservable<T>> replaySupplier(
    ...
    return new BufferedTimedReplaySupplier<>(
        parent, bufferSize, time, unit, scheduler, eagerTruncate);
  }
