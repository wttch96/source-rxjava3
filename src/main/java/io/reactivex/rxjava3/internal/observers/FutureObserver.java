...
/**
 * An Observer + Future that expects exactly one upstream value and provides it via the (blocking)
 * Future API.
 *
...
public final class FutureObserver<T> extends CountDownLatch
  ...
  public void onError(Throwable t) {
    if (error == null) {
      Disposable a = upstream.get();
      if (a != this && a != DisposableHelper.DISPOSED && upstream.compareAndSet(a, this)) {
        error = t;
        countDown();
        return;
      }
