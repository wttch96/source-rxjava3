...
/**
 * A Subscriber + Future that expects exactly one upstream value and provides it via the (blocking)
 * Future API.
 *
...
public final class FutureSubscriber<T> extends CountDownLatch
  ...
  public void onError(Throwable t) {
    if (error == null) {
      Subscription a = upstream.get();
      if (a != this && a != SubscriptionHelper.CANCELLED && upstream.compareAndSet(a, this)) {
        error = t;
        countDown();
        return;
      }
