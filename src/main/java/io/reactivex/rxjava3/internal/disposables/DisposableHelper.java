...
public enum DisposableHelper implements Disposable {
  ...
  /**
   * Atomically sets the field to the given non-null Disposable and returns true or returns false if
   * the field is non-null. If the target field contains the common DISPOSED instance, the supplied
   * disposable is disposed. If the field contains other non-null Disposable, an
   * IllegalStateException is signalled to the RxJavaPlugins.onError hook.
   *
   * @param field the target field
  ...
  /**
   * Atomically replaces the Disposable in the field with the given new Disposable but does not
   * dispose the old one.
   *
   * @param field the target field to change
   * @param d the new disposable, null allowed
   * @return true if the operation succeeded, false if the target field contained the common
   *     DISPOSED instance and the given disposable (if not null) is disposed.
   */
  ...
  /**
   * Atomically tries to set the given Disposable on the field if it is null or disposes it if the
   * field contains {@link #DISPOSED}.
   *
   * @param field the target field
