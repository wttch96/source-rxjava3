...
/**
 * A list implementation which combines an ArrayList with a LinkedList to avoid copying values when
 * the capacity needs to be increased.
 *
 * <p>The class is non final to allow embedding it directly and thus saving on object allocation.
 */
public class LinkedArrayList {
  /** The capacity of each array segment. */
  final int capacityHint;
  /**
   * Contains the head of the linked array list if not null. The length is always capacityHint + 1
   * and the last element is an Object[] pointing to the next element of the linked array list.
   */
  Object[] head;
  /** The tail array where new elements will be added. */
  Object[] tail;
  /**
   * The total size of the list; written after elements have been added (release) and and when read,
   * the value indicates how many elements can be safely read (acquire).
   */
  volatile int size;
