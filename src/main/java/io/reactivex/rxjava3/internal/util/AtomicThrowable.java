...
/**
 * Atomic container for Throwables including combining and having a terminal state via
 * ExceptionHelper.
 *
 * <p>Watch out for the leaked AtomicReference methods!
 */
public final class AtomicThrowable extends AtomicReference<Throwable> {
  ...
  /**
   * Atomically adds a Throwable to this container (combining with a previous Throwable is
   * necessary) or reports the error the global error handler and no changes are made.
   *
   * @param t the throwable to add
   * @return true if successful, false if the container has been terminated
   */
  ...
  /**
   * Atomically terminate the container and return the contents of the last non-terminal Throwable
   * of it.
   *
  ...
  /**
   * Tries to terminate this atomic throwable (by swapping in the TERMINATED indicator) and calls
   * {@link RxJavaPlugins#onError(Throwable)} if there was a non-null, non-indicator exception
   * contained within before.
   *
   * @since 3.0.0
   */
  ...
  /**
   * Tries to terminate this atomic throwable (by swapping in the TERMINATED indicator) and notifies
   * the consumer if there was no error (onComplete) or there was a non-null, non-indicator
   * exception contained before (onError). If there was a terminated indicator, the consumer is not
   * signaled.
   *
