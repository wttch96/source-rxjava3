...
public final class QueueDrainHelper {
  ...
  public static <T, U> void drainLoop(
      SimplePlainQueue<T> q,
      Observer<? super U> a,
      boolean delayError,
      Disposable dispose,
  ...
  /**
   * Creates a queue: spsc-array if capacityHint is positive and spsc-linked-array if capacityHint
   * is negative; in both cases, the capacity is the absolute value of prefetch.
   *
  ...
  /**
   * Requests {@link Long#MAX_VALUE} if prefetch is negative or the exact amount if prefetch is
   * positive.
   *
  ...
  /**
   * Accumulates requests (not validated) and handles the completed mode draining of the queue based
   * on the requests.
   *
   * <p>Post-completion backpressure handles the case when a source produces values based on
   * requests when it is active but more values are available even after its completion. In this
   * case, the onComplete() can't just emit the contents of the queue but has to coordinate with the
   * requested amounts. This requires two distinct modes: active and completed. In active mode,
   * requests flow through and the queue is not accessed but in completed mode, requests no-longer
   * reach the upstream but help in draining the queue.
   *
   ...
   *
   * <p>Don't modify the queue after calling this method!
   *
   * <p>Post-completion backpressure handles the case when a source produces values based on
   * requests when it is active but more values are available even after its completion. In this
   * case, the onComplete() can't just emit the contents of the queue but has to coordinate with the
   ...
   * reach the upstream but help in draining the queue.
   *
   * <p>The algorithm utilizes the most significant bit (bit 63) of a long value (AtomicLong) since
   ...
   * @param isCancelled a supplier that returns true if the drain has been cancelled
   */
  public static <T> void postComplete(
