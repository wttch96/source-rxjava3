...
public enum SubscriptionHelper implements Subscription {
  ...
  /**
   * Verifies that current is null, next is not null, otherwise signals errors to the RxJavaPlugins
   * and returns false.
   *
  ...
  /**
   * Reports that the subscription is already set to the RxJavaPlugins error handler, which is an
   * indication of a onSubscribe management bug.
   */
   ...
   * @return true if the operation succeeded, false if the target field holds the {@link #CANCELLED}
   *     instance.
   ...
   *
   * <p>If the field is not null and doesn't contain the {@link #CANCELLED} instance, the {@link
   * #reportSubscriptionSet()} is called.
   *
  ...
  /**
   * Atomically swaps in the common cancelled subscription instance and cancels the previous
   * subscription if any.
   *
   * @param field the target field to dispose the contents of
   * @return true if the swap from the non-cancelled instance to the common cancelled instance
   *     happened in the caller's thread (allows further one-time actions).
   */
  ...
  /**
   * Atomically sets the new Subscription on the field and requests any accumulated amount from the
   * requested field.
   *
   ...
   */
  public static boolean deferredSetOnce(
      AtomicReference<Subscription> field, AtomicLong requested, Subscription s) {
  ...
  /**
   * Atomically requests from the Subscription in the field if not null, otherwise accumulates the
   * request amount in the requested field to be requested once the field is set to non-null.
   *
   * @param field the target field that may already contain a Subscription
   * @param requested the current requested amount
   * @param n the request amount, positive (verified)
   */
  public static void deferredRequest(
      AtomicReference<Subscription> field, AtomicLong requested, long n) {
