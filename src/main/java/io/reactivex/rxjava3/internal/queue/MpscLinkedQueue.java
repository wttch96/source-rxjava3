...
public final class MpscLinkedQueue<T> implements SimplePlainQueue<T> {
   ...
   * </ol>
   *
   * This works because each producer is guaranteed to 'plant' a new node and link the old node. No
   * 2 producers can get the same producer node as part of XCHG guarantee.
   *
  ...
  public boolean offer(final T e) {
    ...
    // Should a producer thread get interrupted here the chain WILL be broken until that thread is
    // resumed
   ...
   * </ol>
   *
   * This means the consumerNode.value is always null, which is also the starting point for the
   * queue. Because null values are not allowed to be offered this is the only node with it's value
   * set to null at any one time.
   *
