...
/**
 * Base class with shared infrastructure to support {@link
 * io.reactivex.rxjava3.subscribers.TestSubscriber TestSubscriber} and {@link TestObserver}.
 *
...
public abstract class BaseTestConsumer<T, U extends BaseTestConsumer<T, U>> {
   ...
   *
   * <p>Note that accessing the items via certain methods of the {@link List} interface while the
   * upstream is still actively emitting more items may result in a {@code
   * ConcurrentModificationException}.
   *
   * <p>The {@link List#size()} method will return the number of items already received by this
   * {@code TestObserver}/{@code TestSubscriber} in a thread-safe manner that can be read via {@link
   * List#get(int)}) method (index range of 0 to {@code List.size() - 1}).
   *
   * <p>A view of the returned List can be created via {@link List#subList(int, int)} by using the
   * bounds 0 (inclusive) to {@link List#size()} (exclusive) which, when accessed in a read-only
   * fashion, should be also thread-safe and not throw any {@code ConcurrentModificationException}.
   *
   ...
   *
   * <p>Note this is deliberately the only fail method. Most of the times an assertion would fail
   * but it is possible it was due to an exception somewhere. This construct will capture those
   * potential errors and report it along with the original failure.
   *
  ...
  /**
   * Awaits until this {@code TestObserver}/{@code TestSubscriber} receives an {@code onError} or
   * {@code onComplete} events.
   *
   * @return this
   ...
   * @return true if the {@code TestObserver}/{@code TestSubscriber} terminated, false if timeout
   *     happened
   * @throws InterruptedException if the current thread is interrupted while waiting
   */
  ...
  /**
   * Assert that this {@code TestObserver}/{@code TestSubscriber} received exactly one {@code
   * onComplete} event.
   *
   * @return this
   */
  ...
  /**
   * Assert that this {@code TestObserver}/{@code TestSubscriber} has not received an {@code
   * onComplete} event.
   *
   * @return this
   */
  ...
  /**
   * Assert that this {@code TestObserver}/{@code TestSubscriber} received exactly the specified
   * {@code onError} event value.
   *
   * <p>The comparison is performed via {@link Objects#equals(Object, Object)}; since most
   * exceptions don't implement equals(), this assertion may fail. Use the {@link
   * #assertError(Class)} overload to test against the class of an error instead of an instance of
   * an error or {@link #assertError(Predicate)} to test with different condition.
   *
  ...
  /**
   * Asserts that this {@code TestObserver}/{@code TestSubscriber} received exactly one {@code
   * onError} event which is an instance of the specified {@code errorClass} {@link Class}.
   *
  ...
  /**
   * Asserts that this {@code TestObserver}/{@code TestSubscriber} received exactly one {@code
   * onError} event for which the provided predicate returns {@code true}.
   *
   * @param errorPredicate the predicate that receives the error {@link Throwable} and should return
   *     {@code true} for expected errors.
   * @return this
   */
  @NonNull
  ...
  /**
   * Assert that this {@code TestObserver}/{@code TestSubscriber} received exactly one {@code
   * onNext} value which is equal to the given value with respect to {@link Objects#equals(Object,
   * Object)}.
   *
  ...
  /**
   * Asserts that this {@code TestObserver}/{@code TestSubscriber} received exactly one {@code
   * onNext} value for which the provided predicate returns {@code true}.
   *
   * @param valuePredicate the predicate that receives the {@code onNext} value and should return
   *     {@code true} for the expected value.
   * @return this
   */
  @SuppressWarnings("unchecked")
  @NonNull
  public final U assertValue(@NonNull Predicate<T> valuePredicate) {
    ...
    if (values.size() > 1) {
      throw fail(
          "The first value passed the predicate but this consumer received more than one value");
    }
  ...
  /**
   * Asserts that this {@code TestObserver}/{@code TestSubscriber} received an {@code onNext} value
   * at the given index which is equal to the given value with respect to {@code null}-safe {@link
  ...
  /**
   * Asserts that this {@code TestObserver}/{@code TestSubscriber} received an {@code onNext} value
   * at the given index for the provided predicate returns {@code true}.
   *
   * @param index the position to assert on
   * @param valuePredicate the predicate that receives the {@code onNext} value and should return
   *     {@code true} for the expected value.
  ...
  public final U assertValueAt(int index, @NonNull Predicate<T> valuePredicate) {
    ...
    if (!found) {
      throw fail(
          "Value " + valueAndClass(v) + " at position " + index + " did not pass the predicate");
    }
    return (U) this;
  }
  ...
  /**
   * Assert that this {@code TestObserver}/{@code TestSubscriber} received the specified number
   * {@code onNext} events.
   *
  ...
  /**
   * Assert that this {@code TestObserver}/{@code TestSubscriber} has not received any {@code
   * onNext} events.
   *
   * @return this
   */
  ...
  /**
   * Assert that the {@code TestObserver}/{@code TestSubscriber} received only the specified values
  ...
  /**
   * Assert that the {@code TestObserver}/{@code TestSubscriber} received only the specified values
   * in the specified order without terminating.
   *
  ...
  /**
   * Assert that the upstream signaled the specified values in order and then failed with a specific
   * class or subclass of {@link Throwable}.
   *
  ...
  public final U assertFailure(@NonNull Class<? extends Throwable> error, @NonNull T... values) {
    return assertSubscribed().assertValues(values).assertError(error).assertNotComplete();
  }

  /**
   * Awaits until the internal latch is counted down.
   *
   * <p>If the wait times out or gets interrupted, the {@code TestObserver}/{@code TestSubscriber}
   * is cancelled.
   *
   * @param time the waiting time
   * @param unit the time unit of the waiting time
  ...
  /**
   * Assert that the {@code TestObserver}/{@code TestSubscriber} has received a {@link
   * io.reactivex.rxjava3.disposables.Disposable Disposable}/{@link org.reactivestreams.Subscription
   * Subscription} via {@code onSubscribe} but no other events.
   *
   * @return this
   */
  @NonNull
  ...
  /**
   * Await until the {@code TestObserver}/{@code TestSubscriber} receives the given number of items
   * or terminates by sleeping 10 milliseconds at a time up to 5000 milliseconds of timeout.
   *
   * <p>History: 2.0.7 - experimental
   *
